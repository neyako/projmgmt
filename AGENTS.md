...
- `NEXTCLOUD_URL`, `NEXTCLOUD_USER`, `NEXTCLOUD_PASSWORD` - Nextcloud WebDAV access.
- `NEXTCLOUD_BASE_PATH` - active Nextcloud working directory for folder provisioning and draft scanning.
- `NEXTCLOUD_ARCHIVE_PATH` - archive root used when completed folders move to `Done/<year>`.
...
- Review rejection moves the project back to `Editing`, stores `reviewFeedback`, clears `reviewLink`, and increments `draftVersion` with Prisma's atomic `increment`.
...
- Project-facing Server Actions for Nextcloud behavior live in `src/actions/projects.ts` (`createProject`, `scanForDraft`, status/publish/review transitions). Treat `src/actions/nextcloud.ts` as legacy unless a caller explicitly uses it.
